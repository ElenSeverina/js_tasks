...
const newArray02 = array2.filter(item => typeof item === "number" && Number.isFinite(item) || typeof item === "bigint");
const newArray2 = array2.filter(item => typeof item === "object" && item !== null);
...
function filteredWords(str) {
  ...
  words.forEach(word => {
    if (!uniqueWords.some(uniqueWord => uniqueWord.toLowerCase() === word.toLowerCase()) && word !== "") {
...
const strKebabCase = strCamelCase.split(/(?=[A-Z])/).join('-').toLowerCase();
...
function getFlatArray(arr) {
  return arr.reduce((accumulator, current) => {
    return accumulator.concat(Array.isArray(current) ? getFlatArray(current) : current);
  }, []);
}
...
function deepCopy(item) {
  ...
  return Array.isArray(item) 
    ? item.map(deepCopy)
    : Object.fromEntries(Object.entries(item).map(([key, value]) => [key, deepCopy(value)]));
}
